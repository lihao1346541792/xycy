...
<%--<html>
<head>
    <link rel="stylesheet" type="text/css" href="/zgjy_lih/easyui/themes/black/easyui.css">
    <link rel="stylesheet" type="text/css" href="/zgjy_lih/easyui/themes/icon.css">
    <script type="text/javascript" src="/zgjy_lih/easyui/js/jquery.min.js"></script>
    <script type="text/javascript" src="/zgjy_lih/easyui/js/jquery.easyui.min.js"></script>
    &lt;%&ndash;<script type="text/javascript" src="/zgjy_lih/easyui/js/easyui-lang-zh_CN.js"></script>&ndash;%&gt;
    <script type="text/javascript" src="/zgjy_lih/easyui/js/jquery-easyui-validater.js"></script>--%>
    <script type="text/javascript">
        $(function () {
            $('#goto').click(function () {
                location.href="/zgjy_lih/user/getAll";
            });
            $('#dg').datagrid({
                url:"/zgjy_lih/user/showAll"
            });
            ...
            $("#add").click(
                ...
                function openUserDialog(){
                    ...
                    $("#aa").dialog({
                        ...
                        height:300,
                        draggable:false,
                        href:'/zgjy_lih/add.jsp',//easyui在做多个页面引入时 相当于页面的源码包含 1.注意一定保证id唯一 2.子页面不能再出现body head html 等标签(保证父子页面在同一个body中)
                        buttons:[{
                            text:'保存用户',
                            iconCls:'icon-save',
                            handler:function () {
        ...
        function deluser(id){
            ...
            $.messager.confirm('提示','确定要删除'+id+'吗?',function(r){
                ...
                if(r){
                    //发送ajax请求删除用户
                    $.get('/zgjy_lih/user/cancel?id='+id,function(result){
                        $.messager.show({
                            title:'提示',
                            msg:result,
                        });
        ...
        function openUserDialog(id){
            ...
            $.messager.confirm('提示','确定要改变'+id+'的状态吗?',function(r){
                ...
                if(r){
                    //发送ajax请求删除用户
                    $.get('/zgjy_lih/user/update?id='+id,function(result){
                        $.messager.show({
                            title:'提示',
                            msg:result,
                        });
            ...
            /*$("#aa").dialog({
                ...
                href:'/zgjy_lih/upd.jsp?id='+id,//easyui在做多个页面引入时 相当于页面的源码包含 1.注意一定保证id唯一 2.子页面不能再出现body head html 等标签(保证父子页面在同一个body中)
                buttons:[{
                    text:'确认修改',
                    iconCls:'icon-save',
                    handler:function () {
                        $("#ff").form('submit',{
                            url:"/zgjy_lih/user/modify",
                            onSubmit:function () {
                                return true;
                            },
                            success:function () {
        ...
        function qq(value,name){
           $("#dg").datagrid({
                url:"/zgjy_lih/user/showSome?col="+name+"&value="+value
            })
        }
